...
const objLat = (obj) => {
    ...
    const capitalizedFirstName = capitalizeFirstLetter(obj.firstName);
    const capitalizedLastName = capitalizeFirstLetter(obj.lastName);
...
const cvFormatter = (arr) => {
    ...
    const filteredApplicants = arr.filter(applicant => applicant.yearsOfExperience > 1);

    const formattedCvs = filteredApplicants.map(applicant => {
...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if ((!applicant.firstName && !applicant.lastName) || applicant.yearsOfExperience <= 1) {
            rejectedApplicants++;
        } else {
    ...
    return {
        ...
        javaScript_devs: javaScriptDevs,
...
const classesAvg = (data) => {
    ...
    for (let i = 0; i < data.grades.length; i++) {
        ...
        for (let j = 0; j < grade.classes.length; j++) {
            ...
            const avg = Math.floor(sum / currentClass.classScores.length);
            currentClass.avg = avg;
        }
    }
